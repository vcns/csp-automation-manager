...
class GithubUpdateCheckerTest extends TestCase {
	...
	public function test_new_manifest_version_populates_native_update_response(): void {
		$GLOBALS['_wp_remote_get_response'] = $this->response( $this->manifest( '1.0.21' ) );
		...
		$this->assertSame( '1.0.21', $item->new_version );
		$this->assertSame( 'https://vcns.github.io/wp-updates/csp-automation-manager/csp-automation-manager-github-v1.0.21.zip', $item->package );
	...
	public function test_plugin_information_modal_uses_manifest_sections(): void {
		$GLOBALS['_wp_remote_get_response'] = $this->response( $this->manifest( '1.0.20' ) );
		...
		$this->assertSame( '1.0.20', $result->version );
